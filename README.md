...
### Інгест подій
`POST /events` приймає список подій у JSON-форматі.  
Кожна подія має:
- `event_id`
- `occurred_at`
- `user_id`
...
- `/stats/dau/` — щоденна кількість унікальних користувачів (параметри `from`, `to`, підтримує фільтри по `event type`);
- `/stats/top-events/` — топ типів подій (параметри `from`, `to`, `limit`);
...
- **FastAPI-сервіс** приймає запити `/events` і публікує події у NATS.
